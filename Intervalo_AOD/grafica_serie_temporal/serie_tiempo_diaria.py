...
def load_clean_and_resample(filepath, date_range, variables_to_keep, value_replacement, ranges):
    ...
    resampled_data = data.resample('D').agg({'AOD': 'mean', 'PM25': 'mean'}).reset_index()
...
for i, filepath in enumerate(filepaths):
    ...
    if data is not None and not data.empty:
       ...
       # Ajustar los valores del eje X para mostrar solo el año correspondiente
        ax1.xaxis.set_major_locator(mdates.YearLocator())  # Etiquetas en intervalos anuales
        ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))  # Formato solo del año
        ax1.tick_params(axis='x', rotation=90, labelsize=5)  # Rotar etiquetas y ajustar tamaño      
